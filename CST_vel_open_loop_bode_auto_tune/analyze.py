...
def _interp_nan(x):
    """Fill NaNs/inf values by linear interpolation to keep filters stable."""
...
def _hann_smooth(x, fs, cutoff_hz):
    """Simple Hann FIR smoother driven by approximate cutoff."""
    if cutoff_hz is None or cutoff_hz <= 0.0:
        return x.copy()
    fs = float(max(fs, 1e-6))
    samples = fs / max(cutoff_hz, 1e-6)
    win = int(round(samples))
    win = max(win, 3)
    if win % 2 == 0:
        win += 1
...
def fit_mechanical_model(t, torque, velocity, fs=None,
                         torque_scale=1.0, vel_scale=1.0,
                         smooth_hz=150.0, deriv_hz=120.0,
                         vel_deadband=0.0):
    """
    Fit tau = J * dw/dt + B * w + Tc * sign(w) to estimate inertia, damping and friction.
    Returns dict with J, B, Tc, residual statistics, etc.
    """
    t = np.asarray(t, float).ravel()
    torque = _interp_nan(np.asarray(torque, float).ravel()) * float(torque_scale)
    velocity = _interp_nan(np.asarray(velocity, float).ravel()) * float(vel_scale)

    if t.size != torque.size or t.size != velocity.size:
        raise ValueError("Time, torque, and velocity must be same length")
    if t.size < 10:
        raise ValueError("Signals are too short for mechanical fit")

    # Infer sampling frequency if not provided
    if fs is None:
        dt = np.diff(t)
        dt = dt[np.isfinite(dt) & (dt > 0)]
        if dt.size == 0:
            raise ValueError("Unable to infer sampling period from time vector")
        fs_eff = 1.0 / np.median(dt)
    else:
        fs_eff = float(fs)

    v_filt = _hann_smooth(velocity, fs_eff, smooth_hz)
    acc = np.gradient(v_filt, t)
    acc = _hann_smooth(acc, fs_eff, deriv_hz)

    mask = np.isfinite(torque) & np.isfinite(v_filt) & np.isfinite(acc)
    if vel_deadband > 0.0:
        mask &= np.abs(v_filt) >= float(vel_deadband)
    if mask.sum() < 5:
        raise ValueError("Not enough valid samples for least squares fit")

    Phi = np.column_stack([acc[mask], v_filt[mask], np.sign(v_filt[mask])])
    theta = torque[mask]
    coeff, *_ = np.linalg.lstsq(Phi, theta, rcond=None)
    J, B, Tc = coeff
    fitted = Phi @ coeff
    resid = theta - fitted

    res_rms = float(np.sqrt(np.mean(resid**2)))
    torque_rms = float(np.sqrt(np.mean(theta**2)))
    var = float(np.var(theta)) if theta.size > 1 else 0.0
    ...
    return {
        ...
        "samples_used": int(mask.sum()),
    }


def velocity_pi_from_JB(J, B, f_bw=100.0, zeta=1.0):
    """Return (Kp, Ki) for a critically damped PI design based on inertia/damping."""
