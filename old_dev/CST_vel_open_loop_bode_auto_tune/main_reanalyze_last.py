...
if __name__ == "__main__":    
    ...
    # Identify mechanical model tau = J*dw/dt + B*w + Tc*sign(w)
    trq_key = "TRQ_ACT" if "TRQ_ACT" in vals_by_pv else "SP_RBV"
    torque = vals_by_pv[trq_key] * trqScaleOutput
    velocity = vals_by_pv["VEL_ACT"] * velScaleOutput
    mech = analyze.fit_mechanical_model(
        t,
        torque,
        velocity,
        fs=fs,
    ...
    print(f"\nMechanical fit using {trq_key}:")
    print(f"  J  = {mech['J']:.6g} [N·m·s²/rad]")
    print(f"  B  = {mech['B']:.6g} [N·m·s/rad]")
    print(f"  Tc = {mech['Tc']:.6g} [N·m]")
    print(f"  residual RMS = {mech['residual_rms']:.6g} ({mech['variance_explained']*100:.1f}% variance explained)")

    # Recommend velocity-loop PI gains for CSV mode handoff
    kp, ki = analyze.velocity_pi_from_JB(mech["J"], mech["B"], f_bw=100.0, zeta=1.0)
    ti = (kp / ki) if ki > 1e-12 else float("inf")
    print("\nSuggested CST velocity PI (f_bw=100 Hz, ζ=1):")
    print(f"  Kp = {kp:.6g} [N·m/(rad/s)]")
    print(f"  Ki = {ki:.6g} [N·m/rad]")
    if np.isfinite(ti):
        print(f"  Ti = {ti:.6g} s")
